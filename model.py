...
# ===============================================================
class GreenHouse:
    def __init__(self, CO2, VP):
        self.ClimateStates.CO2_Air = CO2
        self.ClimateStates.CO2_Top = CO2
        self.Weather.CO2_Out = CO2

        self.ClimateStates.VP_Air = VP
        self.ClimateStates.VP_Top = VP
        self.Weather.VP_Out = VP
    ...
    class Coefficients:
        class Construction:
            ...
            greenhouse_height = 5.1
            cap_CO2Air = 4.7
            cap_CO2Top = greenhouse_height - cap_CO2Air
            roof_ventilation_area = 1.404E4
            side_ventilation_area = 0
    ...
    def update_T(self, T_air, T_Out, WindSpeed):
    ...
    def MC_ExtAir(self):
        # Equation 8.77
        U_ExtCO2 = self.SetPoints.U_ExtAir
    ...
    def MC_AirCan(self):
        # Equation 9.10
        h_AirCan = 1  # Xem nhu quang hop luon xay ra
        P = self.P()
        R = self.R()
    ...
    def MV_PadAir(self):
        # Vi he thong khong su dung quat
        return 0

    def MV_FogAir(self):
        U_Fog = self.SetPoints.U_Fog
        phi_Fog = self.Coefficients.ActiveClimateControl.phi_Fog
        A_Flr = self.Coefficients.Construction.floor_area
        return U_Fog * phi_Fog / A_Flr

    def MV_BlowAir(self):
        U_Blow = self.SetPoints.U_Blow
    ...
    def MV_AirThScr(self):
        # =============== HEC_AirThScr
        U_ThScr = self.SetPoints.U_ThScr
    ...
    def MV_AirMech(self):
        # ============== Mo hinh nay khong su dung he thong lam mat
        return 0
    ...
    def f_ThScr(self):
        ...
        return U_ThScr * K_ThScr * abs(T_Air - T_Out) ** 0.66 + (1 - U_ThScr) / p_Mean * math.sqrt(
            0.5 * p_Mean * (1 - U_ThScr) * self.Constant.GRAVITY * abs(p_Air - p_Out))
    ...
    # ================ ODE
    # =====================================
    def dx_CO2(self, CO2_Air, CO2_Top):
...
def rk4(dx, CO2_Air, CO2_Top, h):
    CO2 = np.array([CO2_Air, CO2_Top])

    k1 = np.array(dx(CO2[0], CO2[1]))
    k2 = np.array(dx(CO2[0] + 0.5 * h * k1[0], CO2[1] + 0.5 * h * k1[1]))
    k3 = np.array(dx(CO2[0] + 0.5 * h * k2[0], CO2[1] + 0.5 * h * k2[1]))
    k4 = np.array(dx(CO2[0] + h * k3[0], CO2[1] + h * k3[1]))

    return CO2 + (k1 + 2 * k2 + 2 * k3 + k4) * h / 6


def euler(dx, CO2_Air, CO2_Top, h):
    CO2 = np.array([CO2_Air, CO2_Top])
    d_CO2 = np.array(dx(CO2_Air, CO2_Top))

    return CO2 + h * d_CO2


# ===============================================================
# Lay du lieu can thiet tu data
# Tout, Tair de chay model
# CO2air de so sanh voi ket qua thu duoc
# ===============================================================
data_Tout, wind_speed, data_Tair, data_CO2air = takedata.read_data("meteo.csv", "Greenhouse_climate.csv")
data_VPAir = np.asarray([GreenHouse.saturation_vapor_pressure(t) for t in data_Tair])

if __name__ == '__main__':
    model = GreenHouse(data_CO2air[0], data_VPAir[0])
    model.update_T(data_Tair[0], data_Tout[0], wind_speed[0])
    ca = []
    cb = []
    h = 10
    for t in range(0, 2 * 86400, h):
        if t % 300 == 0:
            model.update_T(data_Tair[t // 300], data_Tout[t // 300], wind_speed[t // 300])
            ca.append(model.CO2_Air)
            cb.append(model.VP_Air)
        ff = euler(model.dx_CO2, model.CO2_Air, model.CO2_Top, h)
        gg = euler(model.dx_VP, model.VP_Air, model.VP_Top, h)
    ...
    # ============co2
    plt.subplot(121)
    plt.plot(data_CO2air[0:len(ca)], color="#42f563")
    plt.plot(ca, "red", lw=3)
    plt.legend(["Thực tế", "Mô phỏng"])
    plt.xlabel("Step: 5min")
    plt.title("CO2")

    # ===========vp
    plt.subplot(122)
    # plt.plot(data_VPAir[0:len(cb)])
    plt.plot(cb)
    plt.title("vapor")
    plt.show()